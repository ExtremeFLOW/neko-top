program usrneko
  ...
  ! I suppose in the future, all of this init stuff would be held inside of
  ! "problem" So that we make derived problem types, ie, include heat
  ! transfer etc.
  ! In which case the "problem" should own all the fluid and adjoint solvers
  type(case_t) :: C
  ...
  ! I would also argue that either the design or the "problem" should contain
  ! a way to dump all of the mapped coeficients into both the forward and
  ! adjoint.
  ! Right now, we only have the brinkman term, so it's simple.
  ! But if we have conjugate heat transfer etc, there are more equations
  ! and more coefficients to map.
...
! init_from_json
!------------------------------------------------------------------------------
! - here we would read what type of optimization we're doing,
!   ie, passive scalar y/n?
! - steady/ unsteady? etc
...
!---------------------------------------------------------
  ...
  call adjoint_brinkman%init_from_components( &
       adj%scheme%f_adj_x, adj%scheme%f_adj_y, adj%scheme%f_adj_z, &
       design, &
...
!---------------------------------------------------------
  ...
  call output%fields%assign_to_field(11, &
       objective_function%sensitivity_to_coefficient)
  call output%fields%assign_to_field(12, &
       volume_constraint%sensitivity_to_coefficient)
...
!##############################################################################
  ...
  do while (optimization_iteration .lt. 100)
     ...
     print *, 'VOLUME CONSTRAINT', &
          volume_constraint%objective_function_value, &
     ...
     ! this an appropriate place to sample,
     ! it's the current iteration + the sensitivity
     ...
     associate(x => design%design_indicator%x, df0dx=>design%sensitivity%x, &
       ...
       call design%optimizer%mma_update_cpu( &
