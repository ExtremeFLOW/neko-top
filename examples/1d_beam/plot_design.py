...
def plot_all_iterations():
    ...
    for i, file in enumerate(files):
        ...
        if len(x) > 0:
            ...
            plt.plot(x, h, color=colors[i], label=f'Iter {iteration}', alpha=0.7, linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('design_evolution_all.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_iteration_comparison():
    ...
    for i, iter_idx in enumerate(iterations_to_plot):
        if iter_idx == -1:
            ...
            matching_files = [f for f in files if f.endswith(f'{iter_idx:06d}.txt')]
        ...
        if len(x) > 0:
            iteration = int(file.split('_')[-1].split('.')[0])
            plt.plot(x, h, color=colors[i % len(colors)], 
                    linestyle=line_styles[i % len(line_styles)],
                    label=f'Iter {iteration}', linewidth=3, alpha=0.8)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('design_key_iterations.png', dpi=300)
    plt.show()
...
def plot_convergence():
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('height_statistics.png', dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    if choice == '1':
        ...
        plot_iteration_comparison()
        plot_convergence()
        print("To create animation, run with choice 3")
