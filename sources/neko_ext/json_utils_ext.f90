...
contains
  ...
  function json_key_fallback(json, lookup, root, fallback) result(string)
    ...
    string = trim(adjustl(root)) // '.' // trim(adjustl(lookup))
    if (.not. json%valid_path(string)) then
       string = trim(adjustl(fallback)) // '.' // trim(adjustl(lookup))
    end if
