...
contains
   ...
   subroutine get_facets(C, out, facet_type)
      ...
      do e = 1, N_elem
         do f = 1, 6
            if (C%msh%facet_type(f, e) == facet_type_) then
               N_facets = N_facets + 1
            end if
         end do
      end do
   ...
   subroutine get_facet_nodes(C, element_id, facet_id, nodes)
      ...
      if (allocated(nodes) .and. size(nodes, 2) == N_nodes) then
         nodes = 0.0
   ...
   subroutine estimate_temperature(neko_case)
      ...
      temperature_mean = average_weighted(temperature_local - target_temperature, &
                                          facet_area)

      write (log_buf, '(a,f15.7)') &
         "Outlet area-weighted average temperature deviation: ", temperature_mean
