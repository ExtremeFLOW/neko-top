  ...
  subroutine mma_update_vector(this, iter, x, df0dx, fval, dfdx)
    ...
    case ("cpu")
       if (NEKO_BCKND_DEVICE .eq. 1) then
          call device_memcpy(x%x, x%x_d, this%n, DEVICE_TO_HOST, sync = .false.)
          call device_memcpy(df0dx%x, df0dx%x_d, this%n, DEVICE_TO_HOST, sync = .false.)
          call device_memcpy(fval%x, fval%x_d, this%m, DEVICE_TO_HOST, sync = .false.)
          call device_memcpy(dfdx%x, dfdx%x_d, this%m * this%n, DEVICE_TO_HOST, sync = .true.)
       end if
