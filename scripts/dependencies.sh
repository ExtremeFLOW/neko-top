...
function find_neko() {
    ...
    local requested_neko_commit="ad3eaf8f18b337f283bbc327aaa8f95fe6a1326c"
    local requested_neko_repo="https://github.com/ExtremeFLOW/neko.git"
    ...
    # Pin the Neko revision used by the successful LUMI build on 2026-04-29.
    if [[ -d "$NEKO_DIR/.git" ]]; then
        if [[ "$(git -C "$NEKO_DIR" rev-parse HEAD)" != \
              "$requested_neko_commit" ]]; then
            git -C "$NEKO_DIR" fetch --depth 1 "$requested_neko_repo" \
                "$requested_neko_commit"
            git -C "$NEKO_DIR" checkout --detach -q FETCH_HEAD
            CLEAN_NEKO=true
        fi
    fi
    ...
    if [[ ! -d "$NEKO_LIB" || "$CLEAN_NEKO" == true ]]; then
        ...
        if [[ ! -d "$NEKO_DIR" || $(ls -A $NEKO_DIR | wc -l) -eq 0 ]]; then
            git clone --depth 1 --branch neko-top \
                "$requested_neko_repo" "$NEKO_DIR"
            git -C "$NEKO_DIR" fetch --depth 1 "$requested_neko_repo" \
                "$requested_neko_commit"
            git -C "$NEKO_DIR" checkout --detach -q FETCH_HEAD
    ...
    if [ ! -d "$NEKO_LIB" ]; then
        ...
        error "You can download the source code from:"
        error "\tgit@github.com:gloopydoop/neko.git"
        exit 1
    fi
