...
- Each parameter selects a value based on the current iteration and assigned list of values
...
## 3.3 Global Instance
A global instance of the continuation scheduler is provided as `nekotop_continuation`.
It is initialized in the driver at the beginning of the simulation. Any other module that requires a continuation strategy for one or more of its parameters can register them through this global object.
Note that `nekotop_continuation%update(iter)` is called at each iteration in the optimization loop.
...
```
Reads `optimization.solver.continuation_iterations` in the case file as the defualt frequency of updating all parameters registered in the sheduler. Note that individual frequency for can be registered in the json file as `{parameters_name}_iterations` in the case file.
...
```
`idx = (iter - 1) / iterations_per_value + 1`
`target = values(idx)`
