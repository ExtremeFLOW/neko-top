...
contains
  ...
  subroutine func2(n, m, x, f0val, df0dx, fval, dfdx)
    ...
    fval(1) = (x(1) - 5.0_rp)**2 + (x(2) - 2.0_rp)**2 + (x(3) - 1.0_rp)**2 &
         - 9.0_rp
    fval(2) = (x(1) - 3.0_rp)**2 + (x(2) - 4.0_rp)**2 + (x(3) - 3.0_rp)**2 &
         - 9.0_rp

    !dfdx  = 2*[x(1)-5  x(2)-2  x(3)-1
    !           x(1)-3  x(2)-4  x(3)-3];
    dfdx(1,:) = 2.0_rp*[x(1) - 5.0_rp, x(2) - 2.0_rp, x(3) - 1.0_rp]
    dfdx(2,:) = 2.0_rp*[x(1) - 3.0_rp, x(2) - 4.0_rp, x(3) - 3.0_rp]
